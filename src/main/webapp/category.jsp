...
.btn:hover{ opacity:.92; }
  ...
  <script>
    function fetchCategoryCodeIfExists() {
      const name = document.getElementById("name").value;
      const xhr = new XMLHttpRequest();
      xhr.open("GET", "GetCategoryCodeServlet?name=" + encodeURIComponent(name), true);
    ...
    // ✅ code -> name (was missing in your file)
    function fetchCategoryName() {
      const code = document.getElementById("code").value;
      const xhr = new XMLHttpRequest();
      xhr.open("GET", "GetCategoryNameServlet?code=" + encodeURIComponent(code), true);
      xhr.onreadystatechange = function() {
        if (xhr.readyState === 4 && xhr.status === 200) {
...
<body>
  ...
  <div class="hd">
    <h2>カテゴリーを作成</h2>
    <div class="sub">保存するとこの画面に戻ります。次へでテーブル作成へ進みます。</div>
  </div>
  ...
  <div class="card">
    ...
    <form action="<%= request.getContextPath() %>/CategoryServlet" method="post">
      ...
      <input type="hidden" name="returnTo" value="drawer.jsp?page=category_add">
      ...
      <!-- Category Code -->
      <label>カテゴリーコード:</label>
      <input list="codeList" id="code" name="code" onblur="fetchCategoryName()" required>
      ...
      <datalist id="codeList">
        <%
          ...
          try{
            ...
            rs1 = st1.executeQuery("SELECT code FROM categories ORDER BY code");
            while(rs1.next()){
      ...
      <!-- Category Name -->
      <label>カテゴリー名:</label>
      <!-- ✅ use IfExists (same as Group fix) -->
      <input list="nameList" id="name" name="name" onblur="fetchCategoryCodeIfExists()" required>
      ...
      <datalist id="nameList">
        <%
          ...
          try{
            ...
            rs2 = st2.executeQuery("SELECT DISTINCT name FROM categories ORDER BY name");
            while(rs2.next()){
      ...
      <div class="row">
        ...
        <a class="btn btn-ghost" href="drawer.jsp?page=category_list">戻る</a>
        <a class="btn btn-ghost" href="drawer.jsp?page=table_add">次へ</a>
      </div>
    </form>
  </div>
